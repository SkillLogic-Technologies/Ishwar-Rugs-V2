...
export default function Categories() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get(`${BASE_URL}api/category`, { withCredentials: true });
      ...
      >
        {[...categories, ...categories, ...categories].map((cat) => (
          <SwiperSlide key={cat.slug} className="!w-auto group ">
            <div className="flex flex-col items-center" onClick={() => setLocation(`/category/${cat.slug}`)}>
              <div
              ...
              <p className="mt-3 text-sm sm:text-base md:text-lg font-medium tracking-wide text-black dark:text-white text-center transition-all duration-300
